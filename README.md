...
2. 프로젝트 느낀점
   ...
   >처음으로 팀원들과 같이 모티브프로젝트를 진행하였는데 처음이라서 시행착오가 많이 있었지만 매일 아침에 회의 , 트렐로 , 슬랙으로 진행상황을 공유해 소통의 부재를 최소화할려고 노력을 하였던점.
   >프로젝트를 협업하면서 왜 소통이 중요한지도 깨달았습니다.
   >로그인 , 회원가입할때 validation에 대한 개발지식과 시간이 부족해서 프론트에서 validation을 하지 못한 아쉬움이 있었습니다.
   >validation과 fetch통신에 대해서 좀 더 공부를 하고 개발할때에는 부족하지않는 개발자가 되도록 노력해야겠습니다.
